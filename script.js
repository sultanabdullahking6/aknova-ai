...
// --------------------------------------------------------------------------
// Voice language handling — the app must speak/listen in whichever language
// fits, not always English. Users pick Auto / Urdu / English (saved in
// localStorage); "Auto" also detects the reply's own script so a
// Roman-Urdu-typed message and an Urdu-script message each get the right
// spoken voice, instead of everything defaulting to en-US.
// --------------------------------------------------------------------------

const VOICE_LANG_KEY = 'aknova-ai-voice-lang';

function getVoiceLangPref() {
  try { return localStorage.getItem(VOICE_LANG_KEY) || 'auto'; }
  catch { return 'auto'; }
}

function setVoiceLangPref(value) {
  try { localStorage.setItem(VOICE_LANG_KEY, value); } catch { /* storage unavailable */ }
}

// Language to use for SpeechRecognition (listening). Auto defaults to
// Urdu/Pakistan since that best matches how most users here actually speak,
// while still letting anyone switch to English from the selector.
function getRecognitionLang() {
  const pref = getVoiceLangPref();
  if (pref === 'en-US' || pref === 'ur-PK') return pref;
  return 'ur-PK';
}

// Detects the script of the AI's reply text so SpeechSynthesis (speaking)
// picks a matching voice instead of always using the browser's default
// English voice. Urdu-script text -> ur-PK. Otherwise (Roman
// Urdu/Hinglish or English, both Latin script) falls back to the user's
// chosen preference, defaulting to en-US since most devices only ship an
// English voice for Latin-script text anyway.
...
// Picks the closest available system voice for a language code, since
// utterance.lang alone doesn't guarantee a matching voice exists.
...
if (voiceLangSelect) {
  voiceLangSelect.value = getVoiceLangPref();
  voiceLangSelect.addEventListener('change', () => {
    setVoiceLangPref(voiceLangSelect.value);
  });
}

// Some browsers load the voice list asynchronously; this just warms the
// cache so the first spoken reply can already find a matching voice.
if (window.speechSynthesis) {
  window.speechSynthesis.getVoices();
  window.speechSynthesis.addEventListener('voiceschanged', () => window.speechSynthesis.getVoices());
}

// Chat history is automatically saved for logged-in users.
// Guests can chat normally, but their chats are not persisted after reload.
let conversations = [];      // [{ id, title, messages: [{role, content, image?}] }]
let activeId = null;
...
// --------------------------------------------------------------------------
// Persistent chat storage — login-gated, account-specific browser storage
// --------------------------------------------------------------------------
...
function storageKeyForUser(user) {
  ...
  return identity ? `aknova-ai-chats:${identity}` : null;
}
...
function loadConversationsForUser(user) {
  ...
  try {
    ...
    conversations = Array.isArray(data?.conversations)
      ? data.conversations.filter(c => c && typeof c.id === 'string' && Array.isArray(c.messages))
      : [];
  } catch (err) {
...
function persistAfterChange() {
  if (currentUser) saveConversations();
}
...
function getVisibleConversations() {
  ...
  return conversations.filter(c =>
    String(c.title || 'New chat').toLowerCase().includes(term) ||
    c.messages.some(m => String(m.content || '').toLowerCase().includes(term))
  );
}
...
function renderHistory() {
  if (!currentUser) {
    historyList.innerHTML = '<p class="history-empty">Log in to save and restore your chats.</p>';
    return;
  }
  const visible = getVisibleConversations();
  if (visible.length === 0) {
    historyList.innerHTML = `<p class="history-empty">${historySearchTerm ? 'No matching chats.' : 'Your conversations will show up here.'}</p>`;
    return;
  }
...
(function auth() {
  ...
  function showLoggedIn(user) {
    ...
    if (meta.avatar_url) userAvatar.style.backgroundImage = `url(${meta.avatar_url})`;
  ...
  if (!window.netlifyIdentity) {
    setSaveStatus('Login unavailable');
    return;
  }
...
(function liveVoice(){
  ...
  function speak(text) {
    ...
    const spokenLang = detectSpeechLang(clean);
    const utterance = new SpeechSynthesisUtterance(clean);
    utterance.lang = spokenLang;
    const matchedVoice = pickVoiceForLang(spokenLang);
