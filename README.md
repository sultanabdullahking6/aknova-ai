...
Apna khud ka AI chatbot — Groq API ke upar bana hua, login system, multilingual replies, voice features aur tumhari branding ke saath.
...
**Zaroori: API key kabhi bhi chat mein Claude ko mat bhejo.** Chat history sirf logged-in user ke browser storage mein account-specific key ke saath save hoti hai. Key hamesha seedha Netlify environment variables mein dalna.
...
Login button sidebar ke neeche dikhega. Email/password aur "Log in with Google" dono automatically kaam karenge.
...
- **Naam/branding**: `index.html` mein "AKNOVA-AI" replace karo
...
## ChatGPT jaisa aur behtar banana ho to

Ye already built hain: copy button, regenerate button, stop-generating button, message edit, aur login ke baad chat history automatic save/restore.

Baaki abhi tak nahi bana:
- Dark/light mode toggle (site abhi hamesha dark theme mein hai)

Bata dena chahiye to, add kar dunga.
...
**og-image.png:** ab bana hui hai (`/og-image.png`, 1200x630px, AKNOVA-AI branding ke saath) — WhatsApp/Facebook pe link share karoge to ye preview image dikhegi.
...
## Live Voice Mode
A hands-free voice conversation mode is included. It uses the browser Web Speech API for microphone recognition and SpeechSynthesis for spoken replies while reusing the secure Netlify chat endpoint. Chrome/Edge recommended.

## Live Voice troubleshooting
- Use the deployed HTTPS Netlify URL in Chrome or Edge (not a local file opened with file://).
- When the Live Voice button is pressed, the app immediately asks for microphone permission.
- Choose Allow for the microphone.
- The voice feature uses browser SpeechRecognition + SpeechSynthesis; it is not available in every browser.
- The secure Groq chat endpoint must be deployed under `netlify/functions/chat.js` with `GROQ_API_KEY` set in Netlify environment variables.
